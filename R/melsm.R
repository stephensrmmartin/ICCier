...
.parse_formula <- function(formula, data){
  ...
  mf <- model.frame(f, data,na.action='na.omit',drop.unused.levels = TRUE)
  message(paste0('Dropping ',nrow(mf) - n_orig,' incomplete cases.'))

  group <- model.frame(f,mf,lhs=2,rhs=0)
  ...
  return(list(stan_data=stan_data,group_map = group))
}
