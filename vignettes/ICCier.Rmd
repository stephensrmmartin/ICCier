...
Let $\boldsymbol{m_i}$ represent the observation-level design matrix for the response variable, and $\boldsymbol{M}$ represent a group-level row vector.
...
\end{align*}
$$
When an unconditional model is estimated, the design matrices consist only of a vector of 1s.
...
\end{align*}
$$
The standard deviation, $\hat\sigma_{\epsilon,ij}$ is modeled similarly.
...
All random effects ($\boldsymbol{u}_i^{(\mu)},\boldsymbol{u}_i^{(\sigma)}$) are distributed multivariate normal with mean zero.
However, the covariance matrix depends on the group.
$$
\begin{align*}
	\begin{bmatrix}
		\boldsymbol{u}_i^{(\mu)} \\
	...
	\text{Diag}(\boldsymbol{D_i})' &= \boldsymbol{X}_i\boldsymbol{\eta}
\end{align*}
$$
