...
The mean model is a maximal multilevel (i.e., hierarchical, mixed-effects) model for the mean given the within-group and between-group predictors.
It is "maximal" because all within-group predictors have random coefficients that covary.
When an unconditional ICC is estimated, the mean submodel is an intercept-only model, with group-varying means.
...
Instead, the expected SD is modeled using a log-linear multilevel model from within-group and between-group predictors.
